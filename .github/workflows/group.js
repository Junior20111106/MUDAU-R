/* 
 * Copyright © 2025 thendo
 * This file is part of mudau and is licensed under the GNU GPLv3.
...
cmd: "join",
  ...
  try {
    ...
    if (joinResult) return await m.send('```✓ Rimuru Joined successfully!```');
...
cmd: "gpp|setgcpp",
  ...
  try {
    ...
    if (!botAd) return await m.send("_*✘Rimuru Needs To Be Admin!*_");
    if (text && text === "remove") {
...
cmd: "gname|setgcname",
  ...
  try {
    ...
    var botAd = await isBotAdmin(m);
    if (meta.restrict && !botAd) return await m.send("_*✘Rimuru Needs To Be Admin!*_");
...
const cleanNumber = (user.includes('@') ? user.split('@')[0] : user).replace(/\D/g, '') + '@s.whatsapp.net'
  ...
  try {
    ...
    if (status === '403') {
      ...
      return await m.send('_✘ Rimuru is blocked by the user_');
    } else if (status === '200') {
...
kord({
cmd: "close",
  desc: "close a group to allow only admins to send message",
  fromMe: wtype,
  gc: true,
  adminOnly: true,
  type: "group"
...
kord({
cmd: "open",
  desc: "oepn a group to allow all members to send message",
  fromMe: wtype,
  gc: true,
  adminOnly: true,
  type: "group"
...
kord({
  ...
  if (!text && m.isGroup) {
    ...
    try {
      ...
      return await m.send("✘_*Rimuru Needs To Be Admin!*_");
    }
  }
  ...
  try {
    ...
    const response = `*╭━━★彡 GROUP INFORMATION彡 ★━━╮*
...
_MaxWARN:_ ${isExist.maxwrn}`
      ...
      var mssg = `\`\`\` ╭━━★彡 Available AntiBot config 彡★━━╮ \`\`\`
...
_${pre}antibot off_`
      ...
      var msg = `\`\`\` ╭━━★彡 Available AntiBot config 彡★━━╮ \`\`\`
_${pre}antibot delete_
_${pre}antibot kick_
_${pre}antibot warn 3_
_${pre} antibot status_
...
cmd: "events|gcevent|grpevents",
  ...
  try {
    ...
    const defaultWelcome = `@pp ╭━━★彡 WELCOME 彡★━━╮
┃ Hail, Royal Member @user! ⚜️
┃ You have entered the noble realm of &gname
┃ 
┃ 👥 Total Royals: &count
┃ ⏰ Joined at: &time
┃ INFO:use  *.gcs* fore offical groups 
┃ 
┃Use *.menu* to start and have fun
┃
┃ May your stay be majestic and full of glory!
╰━━━━━━━━━━━━━━━━━━━━╯
    
    const defaultGoodbye = `@pp ╭━━★彡 FAREWELL 彡★━━╮
┃ Alas, Royal Member @user has left the kingdom &gname
┃ 
┃ 👥 Remaining Royals: &count
┃ ⏰ Departed at: &time
┃ 
┃let us all think what you did to the group 
┃ 
┃ May your journey ahead be glorious and victorious!
╰━━━━━━━━━━━━━━━━━━━━╯
    ...
    if (!cmd) {
      ...
      return await m.send(`*_╭━━★彡Group Events Settings彡★━━╮_*
_*Usage:*_
...
${c} off\`\`\``
    ...
    return await m.send(`\`\`\`▸ 彡★ Antilink Enabled: kick\`\`\``)
    }
    else if (cmd === "delete") {
...
_MaxWARN:_ ${isExist.maxwrn}`
      ...
      var mssg = `\`\`\`[╭━━★彡 Available AntiGm config 彡★━━╮ \`\`\`
...
_${pre}antigm off_`
      ...
      var msg = `\`\`\` ╭━━★彡 Available AntiGm config 彡★━━╮ \`\`\`
_${pre}antigm delete_
_${pre}antigm kick_
_${pre}antigm warn 3_
_${pre}antigm status_
...
_Mode:_ ${isExist.mode || "members"}`
      ...
      var mssg = `\`\`\` ╭━━★彡 Available AntiTag config 彡★━━╮ \`\`\`
...
_${pre}antitag off_`
      ...
      var msg = `\`\`\` ╭━━★彡 Available AntiTag config 彡★━━╮ \`\`\`
_${pre}antitag delete_
_${pre}antitag kick_
_${pre}antitag warn 3_
_${pre}antitag admins_
